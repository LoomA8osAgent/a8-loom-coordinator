...
**The spec catalog (0.3) — constraint BEFORE, not refusal after.** The code registry is projected into a catalog plus a byte-capped catalog prompt
...
`integrations/spec-catalog.md`, `tools/gen-catalog.js`.
...
- **A rule whose signal is MEANING gets a FORM, not a bigger regex.** Give it named labels
  (the eight-label brief, the return shape, the trailer vocabulary) so a matcher checks
  presence and structure; what cannot be given a form stays *judgment* and is reviewed by
  name (`RULE-LIVES-IN-PROSE-BECAUSE-NO-MATCHER-READS-MEANING`).
