...
| `judgment-gate.js` | PreToolUse Edit/Write · Bash · Agent/Task · PostToolUse Agent/Task · SessionStart | asks the roster's seams at whichever moment each one claims (`edit` \| `doc` \| `commit` \| `spawn` \| `result` \| `compaction`); PRINTS its engagement state, advises by default, DENIES when engaged and unable to ask — except at the two POST-HOC moments (`result`, `compaction`), where the work has already happened, a refusal cannot un-spend it, and the gate REPORTS and ledgers instead, saying so | `judgment.*` | `enabled:false` ⇒ SILENT no-op |
| `judgment-supervisor.js` | (detached; launched by the gate at the spawn moment) | the OPTIONAL in-flight monitor: three questions over a bounded observation of a running lane, a code-first short-circuit, and a policy that writes `steer-once` / `stop-verdict` PROPOSALS. ⛔ it has no channel to a worker and EXECUTES NOTHING | `judgment.supervisor.*` | **OFF by default** — it spawns a process, and a package must never do that silently |
